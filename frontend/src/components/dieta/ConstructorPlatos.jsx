...
function ConstructorPlatos({ onGuardarPlato, onCerrar }) {
      ...
      if (data.success) {
        ...
        const agrupados = {
          ...
          carbohidrato: alimentos.filter(
            (a) =>
              a.tipo_alimento === "carbohidrato" ||
              a.categoria === "carbohidrato"
          ),
  ...
  const eliminarIngrediente = (id) => {
    setIngredientesSeleccionados(
      ingredientesSeleccionados.filter((ing) => ing.id !== id)
    );
  };
  ...
  const actualizarCantidad = (id, nuevaCantidad) => {
    setIngredientesSeleccionados(
      ingredientesSeleccionados.map((ing) =>
        ing.id === id
          ? { ...ing, cantidad: parseFloat(nuevaCantidad) || 0 }
          : ing
      )
    );
  };
    ...
    const totales = ingredientesSeleccionados.reduce(
      (acc, ing) => {
        ...
        return {
          ...
          carbohidratos:
            acc.carbohidratos + (ing.carbohidratos * cantidad) / 100,
  ...
  const guardarPlato = async () => {
    ...
    try {
      ...
      const dataToSend = {
        nombre: nombrePlato,
        descripcion: descripcionPlato || "Plato personalizado",
        tipo_comida: tipoComida,
        ingredientes: ingredientesSeleccionados.map((ing, index) => ({
          alimento_id: ing.alimentoId,
          cantidad_gramos: ing.cantidad,
        })),
      };

      console.log("📤 Enviando plato al backend:", dataToSend);
      ...
      if (result.success) {
        // Crear objeto para añadir a la dieta actual
        const platoParaDieta = {
          id: result.plato.id,
          tipo: "plato_personalizado",
          nombre: result.plato.nombre,
          descripcion: result.plato.descripcion,
          tipoComida: tipoComida,
          ingredientes: ingredientesSeleccionados,
          totales: {
            calorias:
              parseFloat(result.plato.calorias_totales) ||
              totalesPlato.calorias,
            proteinas:
              parseFloat(result.plato.proteinas_totales) ||
              totalesPlato.proteinas,
            carbohidratos:
              parseFloat(result.plato.carbohidratos_totales) ||
              totalesPlato.carbohidratos,
            grasas:
              parseFloat(result.plato.grasas_totales) || totalesPlato.grasas,
          },
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-gray-800">
                Selecciona ingredientes de cada categoría y crea tu receta
                perfecta
              </p>
            </div>
            <button
          ...
          <div className="mt-4 flex items-center gap-2">
            <div
              className={`flex-1 h-2 rounded-full ${
                paso >= 1 ? "bg-black" : "bg-black/20"
              }`}
            ></div>
            <div
              className={`flex-1 h-2 rounded-full ${
                paso >= 2 ? "bg-black" : "bg-black/20"
              }`}
            ></div>
            <div
              className={`flex-1 h-2 rounded-full ${
                paso >= 3 ? "bg-black" : "bg-black/20"
              }`}
            ></div>
          </div>
              ...
              {ingredientesSeleccionados.length > 0 && (
                <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl p-6 border-2 border-uf-gold">
                  <h3 className="text-xl font-bold text-white mb-4 uppercase flex items-center gap-2">
                    <svg
                      className="w-6 h-6 text-uf-gold"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            value={ing.cantidad}
                            onChange={(e) =>
                              actualizarCantidad(ing.id, e.target.value)
                            }
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-uf-gold/20 to-yellow-600/20 border-2 border-uf-gold rounded-xl p-6">
                ...
                <p className="text-gray-400 mb-4">
                  {descripcionPlato || "Plato personalizado"}
                </p>

                <div className="flex items-center gap-4 mb-4">
                  <span className="bg-gray-800 text-white px-4 py-2 rounded-lg font-bold">
                    {tiposComida[tipoComida].icon}{" "}
                    {tiposComida[tipoComida].nombre}
                  </span>
