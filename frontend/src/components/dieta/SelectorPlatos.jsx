...
function SelectorPlatos({ tipoComida, onSeleccionar, onCerrar }) {
  ...
  const cargarPlatos = async () => {
    try {
      // Mapear nombres de comidas al formato del backend
      const mapeoTipos = {
        'desayuno': 'desayuno',
  ...
  const confirmarSeleccion = () => {
    if (platoSeleccionado) {
      onSeleccionar(platoSeleccionado);
    }
  };
      ...
      <div className="bg-gradient-to-br from-gray-900 to-black max-w-5xl w-full max-h-[95vh] overflow-y-auto rounded-2xl border-2 border-uf-gold shadow-2xl">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-uf-gold to-yellow-600 p-6 z-10">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="text-2xl font-anton font-bold text-black uppercase">Platos Predefinidos</h3>
              <p className="text-gray-800 text-sm">Elige una receta completa de nuestro catálogo</p>
            </div>
            <button
              onClick={onCerrar}
            ...
            <>
              ...
              {cargando ? (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <p className="text-gray-400 text-sm line-clamp-2">{plato.descripcion}</p>
                        </div>
                        <div className="text-right ml-4">
                          <div className="text-2xl font-bold text-uf-gold">{Math.round(plato.calorias_totales)}</div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        <span className="bg-gray-700 text-gray-300 px-2 py-1 rounded text-xs">
                          {iconosDificultad[plato.dificultad]?.icon} {iconosDificultad[plato.dificultad]?.text}
                        </span>
                        <span className="bg-gray-700 text-gray-300 px-2 py-1 rounded text-xs">
                      ...
                      <div className="flex justify-between text-sm pt-3 border-t border-gray-700">
                        <div className="text-center">
                          <div className="text-red-400 font-bold">{Math.round(plato.proteinas_totales)}g</div>
                          <div className="text-gray-500 text-xs">P</div>
                        </div>
                        <div className="text-center">
                          <div className="text-blue-400 font-bold">{Math.round(plato.carbohidratos_totales)}g</div>
                          <div className="text-gray-500 text-xs">C</div>
                        </div>
                        <div className="text-center">
                          <div className="text-yellow-400 font-bold">{Math.round(plato.grasas_totales)}g</div>
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-2">Prueba creando tu propio plato personalizado</p>
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <div className="space-y-6">
                <div className="bg-gray-800 rounded-xl p-6">
                  <h4 className="text-2xl font-bold text-white mb-2">{platoSeleccionado.nombre}</h4>
                  <p className="text-gray-400 mb-4">{platoSeleccionado.descripcion}</p>
                  ...
                  <div className="flex flex-wrap gap-3 mb-4">
                    ...
                    <span className="bg-gray-700 text-white px-3 py-1 rounded-lg">
                      {iconosDificultad[platoSeleccionado.dificultad]?.icon} {iconosDificultad[platoSeleccionado.dificultad]?.text}
                    </span>
                    <span className="bg-gray-700 text-white px-3 py-1 rounded-lg">
                      ⭐ {platoSeleccionado.valoracion_promedio} ({platoSeleccionado.total_valoraciones})
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    <div className="text-center bg-gray-700 rounded-lg p-3">
                      <div className="text-3xl font-bold text-uf-gold">{Math.round(platoSeleccionado.calorias_totales)}</div>
                      <div className="text-gray-400 text-xs">kcal</div>
                    </div>
                    <div className="text-center bg-gray-700 rounded-lg p-3">
                      <div className="text-3xl font-bold text-red-400">{Math.round(platoSeleccionado.proteinas_totales)}g</div>
                      <div className="text-gray-400 text-xs">Proteínas</div>
                    </div>
                    <div className="text-center bg-gray-700 rounded-lg p-3">
                      <div className="text-3xl font-bold text-blue-400">{Math.round(platoSeleccionado.carbohidratos_totales)}g</div>
                      <div className="text-gray-400 text-xs">Carbos</div>
                    </div>
                    <div className="text-center bg-gray-700 rounded-lg p-3">
                      <div className="text-3xl font-bold text-yellow-400">{Math.round(platoSeleccionado.grasas_totales)}g</div>
                      <div className="text-gray-400 text-xs">Grasas</div>
                    </div>
                  </div>

                  {/* INGREDIENTES */}
                  <h5 className="text-lg font-bold text-white mb-3 uppercase">Ingredientes:</h5>
                    ...
                    {platoSeleccionado.ingredientes?.map((ing, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-gray-700 p-3 rounded-lg">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white">{ing.alimento.nombre}</span>
                          <span className="text-gray-500 text-sm">({ing.cantidad_gramos}g)</span>
                        </div>
